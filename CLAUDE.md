...
### GitHub Packages NuGet source and `GITHUB_TOKEN`
- `Vludik.Arbitrage.Events` (referenced by `SubscriptionsService.Domain`) is published to GitHub Packages, not nuget.org. A root `nuget.config` declares both `nuget.org` and a `github` source (`https://nuget.pkg.github.com/directiv13/index.json`), with credentials resolved at restore time via NuGet's `%GITHUB_TOKEN%` environment-variable expansion in `ClearTextPassword` — the token itself is never written to any tracked file.
- The token must be supplied as the `GITHUB_TOKEN` environment variable (a GitHub PAT with `read:packages` scope for the `directiv13` org) wherever `dotnet restore`/`build`/`publish` runs against this solution: locally via the shell environment, or via a project-root `.env` file (see `.env.example`), which Docker Compose auto-loads for `${GITHUB_TOKEN}` substitution in `docker-compose.yml`'s `api.build.args`.
- In the container build, `GITHUB_TOKEN` is a build-time `ARG` declared only in the Dockerfile's `build` stage (inherited by `publish`, which reuses `build`'s filesystem) — the `final` runtime stage never declares this `ARG`, so the token is structurally absent from the published image and any container started from it.
- A missing or invalid `GITHUB_TOKEN` surfaces as `NU1301`/HTTP 401 from `nuget.pkg.github.com` — NuGet doesn't distinguish "unset" from "wrong value" (an unset `%GITHUB_TOKEN%` is sent to GitHub literally, which is also rejected with 401).
